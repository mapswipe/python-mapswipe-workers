...
def load_sample_project_drafts(project_type: int) -> dict:
    ...
    sample_file_path = project_types[project_type] + "_drafts.json"
    sample_file_path = os.path.join(sample_data_dir, sample_file_path)
...
def create_project_draft(project_type: int = 0) -> str:
    ...
    for key, project_draft in project_drafts.items():
        project_ids.append(ref.push(project_draft))
