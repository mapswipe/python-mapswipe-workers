...
def do_mapping(uid, user, project_id, number_of_groups):
    ...
    normal_user = user_management.sign_in_with_email_and_password(
    ...
    groups = user_management.get_firebase_db(
        path, custom_arguments, normal_user["idToken"]
    )
    ...
    for group_id, group in groups.items():
        ...
        custom_arguments = ""
        tasks = user_management.get_firebase_db(
            path, custom_arguments, normal_user["idToken"]
        )
...
def get_firebase_data(project_ids, uid, filename):
    ...
    firebase_data = {"projects": {}, "groups": {}, "results": {}, "users": {}}
...
def create_normal_users(number_of_users):
    ...
    for i in range(0, number_of_users):
        ...
        users[user.uid] = {"email": email, "username": username, "password": password}
...
def test_firebase_functions_users(firebase_data_before, firebase_data_after):
    ...
    for user_id in firebase_data_after["users"].keys():
        ...
        user_task_contribution_count_before = (
            firebase_data_before["users"]
            .get(user_id, {})
            .get("taskContributionCount", 0)
        )
        user_task_contribution_count_after = firebase_data_after["users"][user_id][
            "taskContributionCount"
        ]

        user_group_contribution_count_before = (
            firebase_data_before["users"]
            .get(user_id, {})
            .get("groupContributionCount", 0)
        )
        user_group_contribution_count_after = firebase_data_after["users"][user_id][
            "groupContributionCount"
        ]

        user_project_contribution_count_before = (
            firebase_data_before["users"]
            .get(user_id, {})
            .get("projectContributionCount", 0)
        )
        user_project_contribution_count_after = firebase_data_after["users"][user_id][
        ...
        for project_id in firebase_data_before["projects"].keys():
            for group_id in firebase_data_after["results"][project_id].keys():
                if (
                    user_id
                    in firebase_data_after["results"][project_id][group_id].keys()
                ):

                    try:
                        # check if results already existed
                        firebase_data_before["results"][project_id][group_id][user_id][
                            "results"
                        ]
                    except:
                        ...
                        result_count = len(
                            firebase_data_after["results"][project_id][group_id][
                                user_id
                            ]["results"]
                        )
                        results_task_count += result_count

                        group_number_of_tasks = firebase_data_after["groups"][
                            project_id
                        ][group_id]["numberOfTasks"]
                        ...
                        user_group_contributions = (
                            firebase_data_after["users"][user_id]["contributions"]
                            .get(project_id, {})
                            .get(str(group_id), None)
                        )
                        assert isinstance(
                            user_group_contributions, dict
        ...
        table = [
            ...
            ["result task count", "-", results_task_count],
            [
                "project contribution count",
                user_project_contribution_count_before,
                user_project_contribution_count_after,
            ],
            [
                "group contribution count",
                user_group_contribution_count_before,
        ...
        assert (
            user_project_contribution_count_after
            == user_project_contribution_count_before + results_project_count
        ), f"project contribution count not correct for user: {user_id}"

        assert (
            user_group_contribution_count_after
            == user_group_contribution_count_before + results_group_count
        ), f"group contribution count not correct for user: {user_id}"

        assert (
            user_task_contribution_count_after
            == user_task_contribution_count_before + results_task_count
        ), f"task contribution count not correct for user: {user_id}"


def test_firebase_functions_results(firebase_data_before, firebase_data_after):
    """
    ...
    for project_id in firebase_data_before["projects"].keys():
        ...
        project_result_count_before = firebase_data_before["projects"][project_id].get(
            "resultCount", 0
        )
        project_result_count_after = firebase_data_after["projects"][project_id][
            "resultCount"
        ]

        project_progress_before = firebase_data_before["projects"][project_id].get(
            "progress", 0
        )
        project_progress_after = firebase_data_after["projects"][project_id]["progress"]

        project_required_results_before = firebase_data_before["projects"][project_id][
            "requiredResults"
        ]
        project_required_results_after = firebase_data_after["projects"][project_id][
            "requiredResults"
        ]

        project_contributor_count_before = firebase_data_before["projects"][
            project_id
        ].get("contributorCount", 0)
        project_contributor_count_after = firebase_data_after["projects"][project_id][
            "contributorCount"
        ]
        ...
        for group_id in firebase_data_after["results"][project_id].keys():
            for user_id in firebase_data_after["results"][project_id][group_id].keys():
                ...
                try:
                    ...
                    user_result_count = len(
                        firebase_data_after["results"][project_id][group_id][user_id][
                            "results"
                        ]
                    )
                    ...
                    if (
                        firebase_data_after["groups"][project_id][group_id][
                            "requiredCount"
                        ]
                        >= 0
                    ):

                        group_result_count += firebase_data_after["groups"][project_id][
                            group_id
                        ]["numberOfTasks"]
                        true_result_count += user_result_count

                        group_finished_count_before = firebase_data_before["groups"][
                            project_id
                        ][group_id]["finishedCount"]
                        group_required_count_before = firebase_data_before["groups"][
                            project_id
                        ][group_id]["requiredCount"]
                        group_progress_before = firebase_data_before["groups"][
                            project_id
                        ][group_id]["progress"]
                        group_number_of_tasks_before = firebase_data_before["groups"][
                            project_id
                        ][group_id]["numberOfTasks"]

                        group_finished_count_after = firebase_data_after["groups"][
                            project_id
                        ][group_id]["finishedCount"]
                        group_required_count_after = firebase_data_after["groups"][
                            project_id
                        ][group_id]["requiredCount"]
                        group_progress_after = firebase_data_after["groups"][
                            project_id
                        ][group_id]["progress"]
                        group_number_of_tasks_after = firebase_data_after["groups"][
                            project_id
                        ][group_id]["numberOfTasks"]
                        ...
                        table = [
                            [
                                ...
                                group_number_of_tasks_before,
                                group_number_of_tasks_after,
                            ],
                            [
                                "group finished count",
                                group_finished_count_before,
                                group_finished_count_after,
                            ],
                            [
                                "group required count",
                                group_required_count_before,
                                group_required_count_after,
                            ],
                            [
                                "group progress",
                                group_progress_before,
                                group_progress_after,
                            ],
                        ]
                        ...
                        assert (
                            group_finished_count_after
                            == group_finished_count_before + 1
                        ), f"group finished count not correct for project {project_id}, group {group_id}"

                        assert (
                            group_required_count_after
                            == group_required_count_before - 1
                        ), f"group finished count not correct for project {project_id}, group {group_id}"

                        assert (
                            group_progress_after <= 100
                        ...
                        if group_required_count_after > 0:
                            assert group_progress_after == math.floor(
                                float(group_finished_count_after)
                                / (
                                    float(
                                        group_finished_count_after
                                        + group_required_count_after
        ...
        table = [
            ...
            [
                ...
                project_result_count_before,
                project_result_count_after,
            ],
            [
                "project required results",
                project_required_results_before,
                project_required_results_after,
            ],
            ["project progress", project_progress_before, project_progress_after],
            [
                "project contributor count",
                project_contributor_count_before,
                project_contributor_count_after,
            ],
        ]
        print(tabulate(table, headers=[project_id, "before", "after"]))

        assert (
            group_result_count == true_result_count
        ), f"group result count and true result count not matching for project Id: {project_id}"

        assert group_result_count == (
            project_result_count_after - project_result_count_before
        ), f"wrong result count for project Id: {project_id}"

        assert project_progress_after == math.floor(
            float(project_result_count_after)
            / float(project_required_results_after)
            * 100
        ), f"wrong progress for project Id: {project_id}"

        assert (
            project_progress_after <= 100
        ), f"progress bigger than 100% for project Id: {project_id}"

        if (
            not firebase_data_before["users"]
            .get(user_id, {})
            .get(project_id, {})
            .get("contributions", None)
        ):
            assert (
                project_contributor_count_after == project_contributor_count_before + 1
            )
        else:
