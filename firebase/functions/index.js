...
// ==============================
// duplicated functions for old datapaths
// ==============================
//
...
//
// Gets triggered when new results of the group are written to the database.
exports.counter_old = functions.database.ref('/results/{projectId}/{groupId}/{userId}/').onWrite((snapshot, context) => {
    // if result ref does not contain any data 
    // (e.g. when deletion during transfer_results() occured)
    // do nothing
    if (!snapshot.after.exists()) {
    ...
    // Firebase Realtime Database references
    const groupFinishedCountRef = admin.database().ref('/groups/'+context.params.projectId+'/'+context.params.groupId+'/finishedCount')
    const groupRequiredCountRef = admin.database().ref('/groups/'+context.params.projectId+'/'+context.params.groupId+'/requiredCount')
    const numberOfTasksRef      = admin.database().ref('/groups/'+context.params.projectId+'/'+context.params.groupId+'/numberOfTasks')

    const contributorsCountRef  = admin.database().ref('/projects/'+context.params.projectId+'/contributorCount')

    const taskContributionCountRef      = admin.database().ref('/users/'+context.params.userId+'/taskContributionCount')
    const groupContributionCountRef     = admin.database().ref('/users/'+context.params.userId+'/groupContributionCount')
    const projectContributionCountRef   = admin.database().ref('/users/'+context.params.userId+'/projectContributionCount')
    const contributionsRef              = admin.database().ref('/users/'+context.params.userId+'/contributions/'+context.params.projectId+'/'+context.params.groupId)
    const timeSpentMappingRef           = admin.database().ref('/users/'+context.params.userId+'/timeSpentMapping')

    const timestampRef  = admin.database().ref('/results/'+context.params.projectId+'/'+context.params.groupId+'/'+context.params.userId+'/timestamp')
    const startTimeRef  = admin.database().ref('/results/'+context.params.projectId+'/'+context.params.groupId+'/'+context.params.userId+'/startTime')
    const endTimeRef    = admin.database().ref('/results/'+context.params.projectId+'/'+context.params.groupId+'/'+context.params.userId+'/endTime')
...
//
// Gets triggered when group.requiredCount gets changed
exports.projectCounter_old = functions.database.ref('/groups/{projectId}/{groupId}/requiredCount/').onUpdate((groupRequiredCount, context) => {
    const groupNumberOfTasksRef     = admin.database().ref('/groups/'+context.params.projectId+'/'+context.params.groupId+'/numberOfTasks')
    const projectResultCountRef     = admin.database().ref('/projects/'+context.params.projectId+'/resultCount')
    const projectNumberOfTasksRef   = admin.database().ref('/projects/'+context.params.projectId+'/numberOfTasks')
    ...
    if (groupRequiredCount.after.val() < groupRequiredCount.before.val() && groupRequiredCount.after.val() >= 0) {
        ...
        console.log('/groups/'+context.params.projectId+'/'+context.params.groupId+'/requiredCount/ > 0 or got updated but value did not change: Group progress will not be recalculated')
        return null
    }
})
...
//
// Gets triggered when group.requiredCount gets changed
exports.calcGroupProgress_old = functions.database.ref('/groups/{projectId}/{groupId}/requiredCount/').onUpdate((groupRequiredCount, context) => {

    const groupFinishedCountRef = admin.database().ref('/groups/'+context.params.projectId+'/'+context.params.groupId+'/finishedCount')
    const groupProgressRef      = admin.database().ref('/groups/'+context.params.projectId+'/'+context.params.groupId+'/progress')
    ...
    if (groupRequiredCount >= 0) {
        ...
        console.log('/groups/'+context.params.projectId+'/'+context.params.groupId+'/requiredCount/  > 0: Group progress will not be recalculated')
        return null
    }
})
...
//
// Gets triggered when project.resultCount gets changed.
exports.incProjectProgress_old = functions.database.ref('/projects/{projectId}/resultCount/').onUpdate((projectResultCount, context) => {
    const projectNumberOfTasksRef = admin.database().ref('/projects/'+context.params.projectId+'/numberOfTasks')
    const projectProgressRef      = admin.database().ref('/projects/'+context.params.projectId+'/progress')
...
//
// Gets triggered when project.numberOfTasks gets changed.
exports.decProjectProgress_old = functions.database.ref('/projects/{projectId}/numberOfTasks/').onUpdate((projectNumberOfTasks, context) => {
